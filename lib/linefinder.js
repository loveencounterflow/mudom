(function() {
  ...
  Finder = class Finder {
    ...
    constructor(cfg) {
      ...
      if (this.cfg.inject_stylesheet_after != null) {
        this._inject_stylesheet('after', this.cfg.inject_stylesheet_after);
      }
      if (this.cfg.inject_stylesheet_before != null) {
        this._inject_stylesheet('before', this.cfg.inject_stylesheet_before);
      }
      return void 0;
    }
    ...
    * walk_slugs_of_node(node) {
    ...
    //---------------------------------------------------------------------------------------------------------
    inject_stylesheet_before(element_or_selector) {
      return this._inject_stylesheet('before', element_or_selector);
    }

    inject_stylesheet_after(element_or_selector) {
      return this._inject_stylesheet('after', element_or_selector);
    }
    ...
    _get_stylesheet() {
      ...
      return µ.DOM.new_stylesheet(`.${this.cfg.debug_class_name} iframe {
  outline:                1px dotted red; }

/* ### TAINT use explicit class for debugging line box (as for cover) */
${this.cfg.box_element_name}.${this.cfg.box_class_name} {
  background-color:       transparent;
  pointer-events:         none;
  position:               absolute; }

.${this.cfg.debug_class_name} ${this.cfg.box_element_name}.${this.cfg.box_class_name} {
  background-color:       rgba( 255, 248, 0, 0.2 );
  outline:                1px solid rgba( 255, 0, 0, 0.2 );
  mix-blend-mode:         multiply; }

${this.cfg.box_element_name}.${this.cfg.box_class_name}.${this.cfg.cover_class_name} {
  background-color:       white;
  pointer-events:         none;
  position:               absolute; }

.${this.cfg.debug_class_name} ${this.cfg.box_element_name}.${this.cfg.box_class_name}.${this.cfg.cover_class_name} {
