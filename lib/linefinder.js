...
button#${this.cfg.debug_button_id} {
  ...
  left:                   95mm; }`);
    ...
    //---------------------------------------------------------------------------------------------------------
    insert_debug_button() {
      log('^236473627^', this._get_debug_button());
    ...
    _get_debug_button() {
      ...
      µ.DOM.on(R, 'click', function() {
        var galley_µ, ref1, results, ø_iframe;
        µ.DOM.toggle_class(µ.DOM.select_first('body'), this.cfg.debug_class_name);
        ref1 = distributor.new_iframe_walker();
        results = [];
        for (ø_iframe of ref1) {
          galley_µ = ø_iframe.window.µ;
          results.push(galley_µ.DOM.toggle_class(galley_µ.DOM.select_first('body'), this.cfg.debug_class_name));
        }
        return results;
      });
      return R;
